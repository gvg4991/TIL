...
print('함수는 tuple형식의 하나의 객체로 반환되지만 return a,b처럼 사용할 수 있다')
# (2) 함수에서 return을 작성하지 않으면 None 값을 반환한다. 
print('함수에서 return을 작성하지 않으면 None 값을 반환한다.')
# (3) 함수의 인자(parameter)는 함수 선언시 설정한 값이며, 인수(argument)는 함수 호출시 넘겨주는 값이다. 
print('함수의 인자(parameter)는 함수 선언시 설정한 값이며, 인수(argument)는 함수 호출시 넘겨주는 값이다.')
# (4) 가변 인자를 설정할 때는 인자 앞에서 *을 붙이고, 이때는 함수 내에서 tuple로 처리된다. 
print('가변인자로 받을 때도 tuple로 묶어서 처리됨')
...
def my_mul(a=1,b=1): #기본값은 마지막 인자에 지정해 주어야 하며, 앞의 인풋을 기본값으로 할 경우 젤 뒤로 옮겨준 후 print할때 인자의 '이름='을 입력하여 실행
    return a*b
