...
>> pyinstaller -F -w 파일명.py
- 아이콘 넣기
>> pyinstaller icon='아이콘명.ico' 파일명.py

4. 로깅
참고: https://inma.tistory.com/136
import logging
logging.basicConfig(level = logging.ERROR) #기본이 ERROR
logging.error('에러 로그')
- 로그 파일 저장
logging.basicConfig(filename='C:\\Users/ahnlab_auto_download.log', level=logging.DEBUG)
- stream 및 file에 로그
logger = logging.getLogger(__name__)
streamhandler = logging.StreamHandler()
filehandler = logging.FileHandler('C:\\Users/ahnlab_auto_download.log')
logger.addHandler(streamhandler) #logger에 handler 설정
logger.addHandler(filehandler)
logger.error('SETTING')
- 로그 형식 지정
formatter = logging.Formatter('[%(asctime)s][%(levelname)s | %(filename)s: %(lineno)s] >> %(message)s' )
streamhandler.setFormatter(formatter) #logger에 formatter 설정
