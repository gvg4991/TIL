...
for s in range(4):
    if s == 0 and sem[s]>0:
        sem_list+=(['+']*sem[s])
    elif s == 1 and sem[s]>0:
        sem_list+=(['-']*sem[s])
    elif s == 2 and sem[s]>0:
        sem_list+=(['*']*sem[s])
    elif s == 3 and sem[s]>0:
        sem_list+=(['/']*sem[s])
# print(sem_list)

sem_datas = list(itertools.permutations(sem_list))
dae = -9876543210
so = 9876543210
# print(sem_datas)
for sem_data in sem_datas:
    num_data = copy.copy(num)
    # print(sem_data)
    for i in range(case-1):
        if sem_data[i] == '+':
            num_data[i+1] = num_data[i] + num_data[i+1]
        elif sem_data[i] == '-':
            num_data[i+1] = num_data[i] - num_data[i+1]
        elif sem_data[i] == '*':
            num_data[i+1] = num_data[i] * num_data[i+1]
            ...
            if num_data[i]>0:
                ...
                num_data[i+1] = -1*((-1*num_data[i]) // num_data[i+1])
    if dae < num_data[-1]:
        dae = num_data[-1]
    if so > num_data[-1]:
        so = num_data[-1]
