...
case = int(input())
people = [i for i in range(case)]
datas = [[0] for _ in range(case)]
for row in range(case):
    datas[row] = list(map(int,input().split()))
team = list(itertools.combinations(people,case//2))
# print(team)
...
for start in team:
    # start, link팀 멤버 구분하기
    link = copy.copy(people)
    for p in start:
        link.remove(p)
    # start팀 점수 합
    start_team = list(itertools.permutations(start,2))
    start_sum = 0
    for garo,sero in start_team:
        start_sum += datas[sero][garo]
    # link팀 점수 합
    link_team = list(itertools.permutations(link,2))
    link_sum = 0
    for garo,sero in link_team:
        link_sum += datas[sero][garo]
    # 최소값 찾기
    target = abs(start_sum-link_sum)
    if ans > target:
        ans = target
    if ans == 0:
        break
