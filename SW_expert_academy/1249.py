...
for tc in range(int(input())):
    ...
    while q != []: 
        ...
        # result += datas[ty][tx]
        # distance[ty][tx] = result
        # visited[ty][tx] = 1
        if ty==n-1 and tx==n-1:# and result < ans:
            #ans = result
            ans = distance[ty][tx]
        for delta in range(4):
            ...
            if issafe(new_y,new_x) and distance[new_y][new_x] > distance[ty][tx] + datas[new_y][new_x]:# and ans > result + datas[new_y][new_x]:
                distance[new_y][new_x] = distance[ty][tx] + datas[new_y][new_x]
