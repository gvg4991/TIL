...
# 방향을 45도 반시계 회전을 하면 ↙가 되고, 이 칸에는 3번 물고기가 있다. 물고기가 있는 칸이니 서로 위치를 바꾸고, <그림 4>와 같아지게 된다.
# 3번 물고기의 방향은 ↑이고, 존재하지 않는 칸이다. 45도 반시계 회전을 한 방향 ↖도 존재하지 않으니, 다시 회전을 한다. ← 방향에는 상어가 있으니 또 회전을 해야 한다.
# ↙ 방향에는 2번 물고기가 있으니 서로의 위치를 교환하면 된다. 이런 식으로 모든 물고기가 이동하면 <그림 5>와 같아진다.
# 물고기가 모두 이동했으니 이제 상어가 이동할 순서이다. 상어의 방향은 ↘이고,
# 이동할 수 있는 칸은 12번 물고기가 있는 칸, 15번 물고기가 있는 칸, 8번 물고기가 있는 칸 중에 하나이다. 만약, 8번 물고기가 있는 칸으로 이동하면, <그림 6>과 같아지게 된다.
# 상어가 먹을 수 있는 물고기 번호의 합의 최댓값을 구해보자.
# 입력
# 첫째 줄부터 4개의 줄에 각 칸의 들어있는 물고기의 정보가 1번 행부터 순서대로 주어진다.
# 물고기의 정보는 두 정수 ai, bi로 이루어져 있고, ai는 물고기의 번호, bi는 방향을 의미한다. 방향 bi는 8보다 작거나 같은 자연수를 의미하고, 1부터 순서대로 ↑, ↖, ←, ↙, ↓, ↘, →, ↗ 를 의미한다.
# 출력
...
def eat(y,x):
    global ans, result
    if not issafe(y,x) or not datas[y][x]:
        if ans < result:
            ans = result
        return
    num,drt = datas[y][x]
...
def move(y,x):
    ...
    if datas[next_y][next_x]:
        temp = datas[next_y][next_x]
        datas[next_y][next_x] = datas[y][x]
        datas[y][x] = temp
    else:
        datas[next_y][next_x] = datas[y][x]
        datas[y][x] = 0
...
for row in range(4):
    ...
    data = []
    for col in range(4):
        data.append([d[2*col],d[2*col+1]])
        fish[d[2*col]-1] = [row,col]
    datas.append(data)
# print(datas)
# print(fish)
