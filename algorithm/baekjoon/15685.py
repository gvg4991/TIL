...
tc = int(input())
datas = [[0]*101 for _ in range(101)]
for case in range(tc):
    target = list(map(int,input().split()))
    datas[target[1]][target[0]] = 1
    dic = [target[2]]
    move(dic,target[3])
    # print(dic)
    for d in dic:
        if d == 0:
            ...
            target[1] += 1
        if 0<=target[0]<=100 and 0<=target[1]<=100:
            datas[target[1]][target[0]] = 1
    # print(datas)
ans = 0
for y in range(100):
    for x in range(100):
        if datas[y][x] == 1 and datas[y+1][x+1] == 1 and datas[y+1][x] == 1 and datas[y][x+1] == 1:
            ans += 1
print(ans)
