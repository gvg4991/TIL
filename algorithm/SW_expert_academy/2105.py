...
def bfs(y,x,dt,dept,l):
    dept += 1
    next_y, next_x = y+dy[dt], x+dx[dt]
    if issafe(next_y,next_x) and not datas[next_y][next_x] in l:
        if (dt==0 and next_y<=case-2 and 2<=next_x) or (dt==1 and 2<=next_y and 1<=next_x) or (dt==2 and 1<=next_y and next_x<=case-3):
            target.append([next_y,next_x,dt,dept])
            dessert[next_y][next_x] = l.copy()
            dessert[next_y][next_x] += [datas[next_y][next_x]]

    if dt < 3:
        dt += 1
    next_y, next_x = y+dy[dt], x+dx[dt]
    if issafe(next_y,next_x) and (not datas[next_y][next_x] in l or [next_y,next_x] == [row,col]):
        # if (dt==0 and next_y<=case-2 and 2<=next_x) or (dt==1 and 2<=next_y and 1<=next_x) or (dt==2 and 1<=next_y and next_x<=case-3):
        target.append([next_y,next_x,dt,dept])
        dessert[next_y][next_x] = l.copy()
        dessert[next_y][next_x] += [datas[next_y][next_x]]
...
for tc in range(test):
    ...
    for row in range(case-2):
        for col in range(1,case-1):
            target.append([row,col,0,1])
            dessert = list([[] for __ in range(case)] for _ in range(case))
            dessert[row][col] += [datas[row][col]]
            while target:
                now_y,now_x,dt,dept = target.popleft()
                if [now_y,now_x] == [row,col] and dept > 3 and ans < dept:
                    ans = dept-1
                bfs(now_y,now_x,dt,dept,dessert[now_y][now_x])
