...
# For example, a pen of size 1×4 is different from a pen of size 2×2
...
for n in num:
    if datas.count(n) >= 4:
        can.append(n)
        can.append(n)
    elif datas.count(n) >= 2:
        can.append(n)
# print(can)

result = []
for y in range(len(can)):
    for x in range(y+1,len(can)):
        if can[y]*can[x] >= target:
            result.append((can[y],can[x]))
