# 문제 설명
# 화장실
# 오프라인 필기 테스트의 시험 감독을 맡게 된 코니는 혹시 부정행위가 일어나지는 않을까 시험장을 구석구석 살펴보았다. 지원자들을 매의 눈으로 주시하던 코니는 놓친 부분을 발견했다.
# 바로 화장실이었다! 지원자들이 화장실에 같이 가서 답을 공유하지는 않을까? 걱정이 많은 코니는 한 가지 사실을 깨닫고 마음을 놓을 수 있었다.
# LINE의 화장실은 모든 지원자들을 서로 다른 화장실로 보낼 수 있을 만큼 넉넉하다는 것이었다.
