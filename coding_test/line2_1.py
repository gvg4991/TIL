# 메시지(message) 처리 시간
# 최대 10개의 메시지를 처리하는 메시지 큐(message queue)가 있다. 메시지 큐에 메시지가 쌓이면 이를 순차적으로 최대 10개의 컨슈머(consumer)가 처리한다.
# 메시지마다 처리에 걸리는 시간은 다를 수 있고 하나의 메시지 처리에 걸리는 시간은 최대 100초이다.
...
for message in range(m):
    time = int(input())
    target = result.index(min(result))
    result[target] += time
